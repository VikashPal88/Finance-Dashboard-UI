...
import { z } from "zod";
import { Loader2, Wallet, PiggyBank, X, Save, Target } from "lucide-react";
...
type EditAccountInput = z.infer<typeof editAccountSchema>;
...
export function EditAccountDrawer({
  ...
  useEffect(() => {
    if (account) {
      ...
      setMonthlyBudget(String(account.monthlyBudget ?? 0));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrors({ _form: errData?.error || "Failed to update account" });
            ...
            <div className="flex items-center justify-between px-6 pb-5 border-b border-gray-200 dark:border-neutral-800">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-neutral-400 mt-1">
                  Update your account details, balance, and budget
                </p>
              </div>
              <button
                onClick={onClose}
